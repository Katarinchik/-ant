...
WAY_TYPE AntColonyOptimization(double** distance0, int vertex, int start, int h, int z) {
	...
	for (int t = 0; t < T_MAX; ++t) {
		...
		for (int k = 0; k < M; ++k) {
			...
			do {
				...
				//cout << hours << ' ' << h << ' ' << j_max << ' '<< ants[k].days<<'\n';
		...
		for (int i = 0; i < way.itabu - 1; ++i) {
			...
			if (pheromone[from][to] + Q / way.length < 200)
			{
...
int main() {
	...
	int z = F[A - 1];
	// запускаем алгоритм
	WAY_TYPE way = AntColonyOptimization(D, N, --A, h * h1, z);
